...
public class G_MatcherScanner {
    ...
    public void g3_vowelTrigraphs() {
        ...
        String result = TRIGRAPH_PAT.matcher(SONNET)
                                    .replaceAll(
                                            matchResult -> "[" + matchResult.group().toUpperCase() + "]");
...
// ========================================================
     ...
     */
    private static final Pattern WORD_PAT = Pattern.compile("\\b[a-z]+'[a-z]{2,}\\b", Pattern.CASE_INSENSITIVE);
    ...
    public void z_readFileIntoString() throws IOException {
        SONNET = Files.readString(Paths.get("SonnetI.txt"));
    }
}
